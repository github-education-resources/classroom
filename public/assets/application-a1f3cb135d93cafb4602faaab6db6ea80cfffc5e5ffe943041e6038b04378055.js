...
(function() {
  var $button, buttonOut, buttonOver, zeroClipboardClient;

  buttonOver = function() {
    var aria_label;
    this.classList.remove('tooltipped', 'tooltipped-s');
    aria_label = $(this).attr('aria-label');
    return $('#global-zeroclipboard-html-bridge').addClass('tooltipped tooltipped-s').attr('aria-label', aria_label || 'Copy to clipboard.');
  };
  ...
  zeroClipboardClient.on('ready', function() {
    zeroClipboardClient.on('copy', function(event) {
      var url;
      url = $('.js-url-field').val().toString();
      return event.clipboardData.setData('text/plain', url);
    });
    return zeroClipboardClient.on('aftercopy', function() {
      $('#global-zeroclipboard-html-bridge').attr('aria-label', 'Copied!');
      return $button.mouseleave(function() {
        return $('#global-zeroclipboard-html-bridge').attr('Copy to clipboard');
      });
    });
  });

  zeroClipboardClient.on('error', function() {
