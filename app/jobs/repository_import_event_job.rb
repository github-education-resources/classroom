...
class RepositoryImportEventJob < ApplicationJob
  ...
  def handle_assignment_repo(assignment_repo, status)
    ...
    when "success"
      ...
      broadcast_assignment_repo_success(user, invite_status, assignment_repo)
      GitHubClassroom.statsd.increment("v3_exercise_repo.import.success")
    when "failure"
      invite_status.errored_importing_starter_code!
      broadcast_assignment_repo_failure(user, invite_status, assignment_repo)
      GitHubClassroom.statsd.increment("v3_exercise_repo.import.failure")
    end
  end

  def broadcast_assignment_repo_success(user, invite_status, assignment_repo)
  ...
  def broadcast_assignment_repo_failure(user, invite_status, assignment_repo)
    ActionCable.server.broadcast(
      RepositoryCreationStatusChannel.channel(user_id: user.id),
      error: IMPORT_FAILED,
      status: invite_status.status,
