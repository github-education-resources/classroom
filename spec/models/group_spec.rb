...
RSpec.describe Group, type: :model do
  ...
  describe "group_invite_statuses", :vcr do
    ...
    let(:invitation_1)   { create(:group_assignment_invitation) }
    let(:invitation_2)   { create(:group_assignment_invitation) }

    it "returns a list of invite statuses" do
      group_invite_status = GroupInviteStatus.create(group: group, group_assignment_invitation: invitation_1)
    ...
    it "on #destroy destroys invite status and not the invitation" do
      ...
      expect(invitation_1.reload.nil?).to be_falsey
    end

    it "on #destroy destroys all invite statuses" do
      group_invite_status_1 = GroupInviteStatus.create(group: group, group_assignment_invitation: invitation_1)
      group_invite_status_2 = GroupInviteStatus.create(group: group, group_assignment_invitation: invitation_2)
      group.destroy
      expect { group_invite_status_1.reload }.to raise_error(ActiveRecord::RecordNotFound)
      expect { group_invite_status_2.reload }.to raise_error(ActiveRecord::RecordNotFound)
    end
  end
end
