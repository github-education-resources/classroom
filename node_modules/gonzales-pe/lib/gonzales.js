...
/***/ function(module, exports, __webpack_require__) {
	...
	*/function getMixin2(){var startPos=pos;var x=[getOperator()];x=x.concat(getSC());if(checkIdentOrInterpolation(pos))x=x.concat(getIdentOrInterpolation());x=x.concat(getSC());if(checkBlock(pos))x.push(getBlock());else{if(checkArguments(pos))x.push(getArguments());x=x.concat(getSC());x.push(getBlock());}var token=tokens[startPos];return newNode(NodeType.MixinType,x,token.ln,token.col);}/**
	 ...
	 * @param {number} i Token's index number
	 * @returns {number} Length of URI
	 */function checkUri(i){var start=i;var l=void 0;if(i>=tokensLength||tokens[i].value!=='url')return 0;// Skip `url`
	i++;if(i>=tokensLength||tokens[i].type!==TokenType.LeftParenthesis)return 0;// Store the opening parenthesis token as we will reference it's `right`
	// property to determine when the parentheses close
	var leftParenthesis=tokens[i];// Skip `(`
	i++;// Determine the type of URI
	while(i<leftParenthesis.right){if(l=checkUri1(i)){i+=l;tokens[start].uriType=1;// Raw based URI (without quotes)
	}else if(l=checkUri2(i)){i+=l;tokens[start].uriType=2;// Non-raw based URI (with quotes)
	}else return 0;}return i-start;}/**
	 ...
	 * @param {number} i Token's index number
	 * @return {number} Length of raw node
	 */function checkUriRaw(i){var start=i;var l=void 0;while(i<tokensLength){if(checkInterpolation(i)||checkVariable(i))break;else if(l=checkUriRawCharacters(i))i+=l;else break;}tokens[start].uri_raw_end=i;return i-start;}/**
	 * Get a raw node
	 * @return {Node}
	 */function getUriRaw(){var startPos=pos;var type=NodeType.RawType;var token=tokens[startPos];var line=token.ln;var column=token.col;var content=[];var l=void 0;while(pos<tokens[startPos].uri_raw_end){if(checkInterpolation(pos)||checkVariable(pos))break;else if(l=checkUriRawCharacters(pos))pos+=l;else break;}content=joinValues(startPos,pos-1);return newNode(type,content,line,column);}/**
	 ...
	 */function checkUri1(i){var start=i;var l=void 0;if(l=checkSC(i))i+=l;while(i<tokensLength){if(l=checkInterpolation(i)||checkUriRaw(i))i+=l;else break;}if(l=checkSC(i))i+=l;// Check that we are at the end of the uri
	if(i<tokens[start-1].right)return 0;tokens[start].uri_end=i;return i-start;}/**
	 * Get a raw (without quotes) URI
	  node
	 * @return {Node}
	 */function getUri1(){var startPos=pos;var type=NodeType.UriType;var token=tokens[startPos];var line=token.ln;var column=token.col;var content=[];var end=void 0;// Skip `url` and `(`
	pos+=2;if(checkSC(pos))content=content.concat(getSC());while(pos<tokens[startPos+2].uri_end){if(checkInterpolation(pos))content.push(getInterpolation());else if(checkUriRaw(pos))content.push(getUriRaw());else break;}if(checkSC(pos))content=content.concat(getSC());// Check that we are at the end of the uri
	if(pos<tokens[startPos+1].right)return 0;end=getLastPosition(content,line,column,1);// Skip `)`
	 ...
	 * @param {number} i Token's index number
	 * @return {number} Length of URI node
	 */function checkUri2(i){var start=i;var l=void 0;while(i<tokensLength){if(l=checkSC(i))i+=l;else if(l=checkString(i))i+=l;else if(l=checkFunction(i))i+=l;else if(l=checkUnary(i))i+=l;else if(l=checkIdentOrInterpolation(i))i+=l;else if(l=checkVariable(i))i+=l;else break;}tokens[start].uri_end=i;return i-start;}/**
	 * Get a non-raw (with quotes) URI node
	 * @return {Node}
	 */function getUri2(){var startPos=pos;var token=tokens[startPos];var line=token.ln;var column=token.col;var content=[];var end=void 0;// Skip `url` and `(`
	pos+=2;while(pos<tokens[startPos+2].uri_end){if(checkSC(pos))content=content.concat(getSC());else if(checkUnary(pos))content.push(getUnary());else if(_checkValue(pos))content.push(_getValue());else break;}end=getLastPosition(content,line,column,1);// Skip `)`
	pos++;return newNode(NodeType.UriType,content,line,column,end);}/**
	 * Check if token is part of a value
	 * @param {number} i Token's index number
	 * @return {number} Length of the value
...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 * @param {number} i Token's index
	 * @return {number} Wildcard length
	 */function _checkUnicodeWildcard(i){var start=i;if(i>=tokensLength)return 0;while(i<tokensLength){if(tokens[i].type===TokenType.QuestionMark)i+=1;else break;}tokens[start].uri_raw_end=i;return i-start;}/**
	 * Check if token is part of URI, e.g. `url('/css/styles.css')`
	 * @param {number} i Token's index number
	 * @returns {number} Length of URI
	 */function checkUri(i){var start=i;var l=void 0;if(i>=tokensLength||tokens[i].value!=='url')return 0;// Skip `url`
